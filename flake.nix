{
  ...
  inputs = {
    ...
    homebrew-sane-apps = {
      url = "github:sane-apps/homebrew-tap"; # sanebar
      flake = false;
    };
  };
  ...
  in {
    ...
    # per-system checks: build each machine config. static gates (fmt/lint/secrets) live in mise (`mise run check`).
    # ci build these per-runner (see .github/workflows/check.yml).
    checks = {
      aarch64-darwin = {
    ...
    in {
      default = pkgs.mkShellNoCC {
        ...
        # install git hooks on first entry, idempotent.
        shellHook = ''
