{lib, ...}: {
  programs.starship = {
    ...
    # https://starship.rs/config/  (colours come from the catppuccin terminal
    # palette, so standard names render in your theme).
    settings = {
      ...
      # know when you're inside a `nix develop` / nix-shell
      nix_shell = {
        symbol = " ";
        style = "bold sky";
      ...
      # Only shown inside k8s/infra dirs.
      kubernetes = {
        disabled = false;
        symbol = "󱃾 ";
        style = "bold red";
