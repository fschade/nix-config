{
  ...
  programs.git = {
    ...
    settings = {
      ...
      rebase.autosquash = true; # apply git-absorb `fixup!` commits on rebase
      rebase.autostash = true; # stash/pop a dirty tree around rebase and pull
      fetch.prune = true; # drop local refs for branches deleted on remote
      ...
      diff.algorithm = "histogram"; # clearer diffs than the default
      branch.sort = "-committerdate"; # most recent used branches first
      column.ui = "auto"; # columns for `git branch` / `git status`
    };
  };
